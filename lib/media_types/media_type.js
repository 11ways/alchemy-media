...
MediaType.setMethod(function supportsWebp(conduit) {
	...
	if (conduit.headers.accept && conduit.headers.accept.indexOf('image/webp') !== -1){
		...
		if (typeof agent.satisfies != 'function') {
			console.log('Satisfies is not a function?', conduit.headers, agent, is, uaString);
		} else {
...
MediaType.setMethod(function getDpr(conduit) {
	...
	var parameter = Number(conduit.param('dpr')),
	    dpr;
	...
	if (!dpr) {
		...
		if (resolution && resolution.dpr && resolution.dpr < 10) {
			dpr = resolution.dpr;
		}
...
MediaType.setMethod(function thumbnail(conduit, record) {
	...
	if (record.MediaRaw && record.MediaRaw.mimetype) {
		extension = mime.getExtension(record.MediaRaw.mimetype);
	} else if (record.path) {
		extension = path.extname(record.path);
	}
...
MediaType.setMethod(function serve(conduit, record) {
	...
	if (!record) {
		throw new Error('Unable to serve file: no record given');
	}

	conduit.serveFile(record.path, {
		mimetype : record.MediaRaw.mimetype,
