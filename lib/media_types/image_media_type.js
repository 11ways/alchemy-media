	...
	if (query.width || query.height) {
		...
		if (query.width) {
			...
			if (String(query.width).indexOf('%') > -1) {
				baseWidth = Number(resolution.width) || query.maxWidth || 1024;
		...
		if (query.height) {
			height = parseInt(query.height) || 768;

			if (String(query.height).indexOf('%') > -1) {
				baseHeight = Number(resolution.height) || query.maxHeight || 768;
	...
	tasks.stripped = function stripData(next) {
		...
		alchemy.copyFile(filePath, tempFile, function madeTempFile(err) {
			...
			child.execFile(that.exivPath, ['rm', tempFile], function afterStrip(err) {
				...
				alchemy.getFileInfo(tempFile, {hash: that.hashType}, function(err, stripped_info) {
