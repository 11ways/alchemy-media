...
alchemy.create('MediaType', function ImageMediaType() {
	...
	var exivPath  = alchemy.plugins.media.exiv2,
	    ...
	    veronica  = new Veronica({temp: alchemy.plugins.media.scratch, cache: alchemy.plugins.media.cache});
	...
	this.serve = function serve(render, record) {
		...
		if (query.profile) {
			...
			if (query.maxWidth) {
				resizeOptions.maxWidth = query.maxWidth;
			}

			if (query.maxHeight) {
				resizeOptions.maxHeight = query.maxHeight;
			}
		}
