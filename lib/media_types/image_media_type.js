 ...
 * @version  1.0.0
 */
var ImageMedia = Function.inherits('MediaType', function ImageMediaType() {
	ImageMediaType.super.call(this);
});
...
ImageMedia.setMethod(function thumbnail(conduit, record) {
	...
	var dimension = 100 * this.getDpr(conduit),
	    options   = {};
	...
	this.veronica.getResizedImage(record.MediaFile.path, options, function gotResizedImage(err, resizedPath) {
...
ImageMedia.setMethod(function serve(conduit, record) {
	...
	if (record == null) {
		return conduit.notFound('No image requested');
	}

	if (record.MediaFile) {

		if (record.MediaFile.path) {
			path = record.MediaFile.path;
		} else {
			return conduit.error(new Error('Record has no file attached'));
		}
	} else {
		record = record.replace('..', '');
		path = libpath.join(APP_ROOT, record);
	}
	...
	if (query.profile) {
		...
		if (query.width) {
			...
			if (String(query.width).indexOf('%') > -1) {
				baseWidth = Number(resolution.width) || query.maxWidth || 100;
		...
		if (query.height) {
			...
			if (String(query.height).indexOf('%') > -1) {
				baseHeight = Number(resolution.height) || query.maxHeight || 100;
	...
	if (resizeOptions) {
		...
		this.veronica.getResizedImage(path, resizeOptions, function gotResizedPath(err, resizedPath) {
			...
			conduit.serveFile(resizedPath, {mimetype: mimetype});
		});
	} else {
...
ImageMedia.setMethod(function placeholder(conduit, options) {
	...
	this.veronica.placeholder(options, function(err, filepath) {
	...
	tasks.stripped = function stripData(next) {
		...
		alchemy.copyFile(filePath, tempFile, function madeTempFile(err) {
			...
			child.execFile(that.exivPath, ['rm', tempFile], function afterStrip(err) {
				...
				alchemy.getFileInfo(tempFile, {hash: this.hashType}, function(err, stripped_info) {
...
ImageMedia.setMethod(function getImageInfo(filePath, callback) {
	...
	child.execFile(this.exivPath, [filePath], function afterStrip(err, out) {
