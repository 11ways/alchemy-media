 ...
 * @version  1.0.0
 */
var ImageMedia = Function.inherits('MediaType', function ImageMediaType(options) {
...
ImageMedia.setMethod(function serve(conduit, record) {
	...
	if (resizeOptions) {
		...
		this.veronica.getResizedImage(path, resizeOptions, function gotResizedPath(err, resizedPath) {
			...
			conduit.serveFile(resizedPath, {mimetype: mimetype, onError: onError, disposition: false});
		});
	} else {
		conduit.serveFile(path, {onError: onError, disposition: false});
	}
});
